...
def get_global_counts(base_dir, whitelist):
    ...
    global_count = np.zeros([IMG_HEIGHT * IMG_WIDTH, N_BINS_R * N_BINS_G * N_BINS_B])
    for id in whitelist:
...
def save_global_hist_shards(global_hist, whitelist_post, shard_id):
    ...
    filename_hist = FLAGS.global_hist_shards_dir + '/{}/pixels_hist.npz'.format(shard_id)
    np.savez_compressed(filename_hist, global_hist)
    filename_whitelist = FLAGS.global_hist_shards_dir + '/{}/whitelist.csv'.format(shard_id)
    df = pd.DataFrame()
...
def sum_global_hist():
    ...
    shard = shards[0]
    global_hist = np.load(FLAGS.global_hist_shards_dir + '/{}/pixels_hist.npz'.format(shard))['arr_0']
    for shard in shards[1:]:
        hist = np.load(FLAGS.global_hist_shards_dir + '/{}/pixels_hist.npz'.format(shard))['arr_0']
